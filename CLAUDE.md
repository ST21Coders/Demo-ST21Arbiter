...
3. **Agent plane** ([`agents/`](agents/)) — five Bedrock AgentCore Runtimes, each a Strands `Agent` wrapped in `bedrock_agentcore.runtime.BedrockAgentCoreApp` on port 8080. Master orchestrator fans out via tools (`sharepoint_lookup`, `awsconfig_lookup`, `zscaler_lookup`, `jira_lookup`) to four specialists. The `jira_specialist` is MCP-based: it uses the Strands `MCPClient` to spawn the `mcp-atlassian` server over stdio (creds from Secrets Manager `dev/<project>/jira`), unlike the other three which call boto3/KB directly. Default foundation model for all runtimes is **Amazon Nova 2 Lite** (`us.amazon.nova-2-lite-v1:0`) — first-party, no AWS Marketplace subscription required. Each agent reads `MODEL_ID` from its runtime env, so you can override per-runtime (e.g. `MASTER_MODEL_ID=us.anthropic.claude-sonnet-4-6` on a `deploy_agents.py` run) once Marketplace terms for Anthropic models are accepted. Specialists hit the Bedrock KB; KB indexes from `s3://dev-<project>-processed/` into an OpenSearch Serverless vector collection. All Bedrock calls go through a Guardrail (content/PII/denied-topics). Master uses AgentCore Memory for long-term conversation continuity, mirrored to the DDB `sessions` table.
...
- **MCP sidebar routes per-agent:** [`MCPChat.jsx`](ui/src/pages/MCPChat.jsx)'s `MCP_SERVERS` array maps each card to a real AgentCore runtime (`sharepoint`/`zscaler`/`awsconfig`/`jira`, plus a static `servicenow` placeholder). Selecting a card and chatting sends `target: <id>` to `sendChat()`; [`api_handler.py`](Infra/functions/api_handler/api_handler.py) `_handle_chat` resolves `target` → that specialist's runtime ARN (no target / unknown → master orchestrator fan-out, which the Analyst page relies on). Live status comes from `GET /agent-status` (`bedrock-agentcore list-agent-runtimes`) via the `useAgentStatus()` hook.
